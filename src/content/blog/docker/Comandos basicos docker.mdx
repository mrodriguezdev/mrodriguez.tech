...
<CodeBlock lines={["docker --version"]} />
...
<CodeBlock lines={["docker images"]} />

### 4. docker run

Es uno de los comandos más importantes. Permite iniciar un contenedor a partir de una imagen.  
Puedes añadir varias opciones al comando <InlineCode text="docker run" />, pero en su forma más simple, basta con especificar el nombre de la imagen.
  ...
  items={[
    {
      title: "-d",
      description:
        "Ejecuta el contenedor en modo de fondo. Esto significa que el contenedor funcionará en segundo plano, permitiéndote seguir usando la terminal para otros comandos. Si no usas -d, el contenedor se ejecutará en primer plano y ocupará la terminal hasta que lo detengas manualmente.",
    },
    {
      title: "-p",
      description:
        "Permite exponer puertos desde el contenedor al host. Esto significa que puedes mapear un puerto del contenedor a un puerto específico en el sistema host, lo que permite acceder a los servicios del contenedor desde fuera de él. La sintaxis es -p <puerto_host>:<puerto_contenedor>.",
    },
  ]}
  listType="bulleted"
/>

Por ejemplo, si tienes una aplicación que escucha en el puerto 80 dentro del contenedor (como Nginx), puedes exponer ese puerto en el puerto 8080 de tu host, de modo que puedas acceder a la aplicación en <InlineCode text="http://localhost:8080" />.
  ...
  items={[
    {
      title: "-d",
      description:
        "Indica que el contenedor debe ejecutarse en segundo plano.",
    },
    {
      title: "-p 8080:80",
      description:
        "Expone el puerto 80 del contenedor en el puerto 8080 del host. Esto permite que cualquier solicitud a http://localhost:8080 se redirija al puerto 80 dentro del contenedor, donde Nginx está escuchando.",
    },
  ]}
  listType="bulleted"
/>
...
Este comando muestra una lista de todos los contenedores activos en el sistema. Para ver todos los contenedores (incluidos los detenidos) usa <InlineCode text="docker ps -a" />.
...
Si necesitas detener un contenedor en ejecución, usa <InlineCode text="docker stop" /> seguido del ID o nombre del contenedor. Esto lo apagará de manera segura.
...
Para obtener el <InlineCode text="<container_id>" />, puedes usar el comando <InlineCode text="docker ps" />.
...
Si necesitas crear una imagen a partir de un <InlineCode text="Dockerfile" />, este comando te permite construirla.  
La opción <InlineCode text="-t" /> te permite etiquetar la imagen con un nombre personalizado.
...
La opción <InlineCode text="-it" /> abre una sesión interactiva dentro del contenedor, permitiéndote ejecutar comandos directamente como si estuvieras dentro de la máquina.
