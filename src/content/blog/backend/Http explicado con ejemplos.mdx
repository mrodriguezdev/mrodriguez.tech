...
Por ejemplo, eliminar el mismo recurso varias veces con <InlineCode text="DELETE" /> no lo borra más de una vez.
...
Ejemplos de uso con la herramienta <InlineCode text="curl" />:
...
Por ejemplo, si mandás JSON pero no ponés <InlineCode text="Content-Type: application/json" />, el servidor podría ignorar completamente el cuerpo de tu solicitud.
...
Y algo clave: no todos los métodos permiten body. Por ejemplo, <InlineCode text="GET" /> no debería usarse con cuerpo, aunque técnicamente algunos clientes lo permitan.
...
Si querés practicar este tipo de solicitudes de forma más visual, además de <InlineCode text="curl" /> o Postman, te recomiendo probar <ExternalLink href="https://cartero.danirod.es/" text="Cartero" />, un cliente HTTP de escritorio liviano, creado como proyecto open source.
